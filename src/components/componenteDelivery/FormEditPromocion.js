  ...
  useEffect(() => {
    if (promocionSelected) {
      reset({
        ...
        fecha_inicio: promocionSelected.fecha_inicio
          ? String(promocionSelected.fecha_inicio).substring(0, 10)
          : "",
        fecha_fin: promocionSelected.fecha_fin
          ? String(promocionSelected.fecha_fin).substring(0, 10)
          : "",
      });

      const platoRelacionado = platos?.find(
        (p) => p.id === parseInt(promocionSelected.idPlato),
      );
      if (platoRelacionado) {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      const response = await axiosInstance.post(
        `delivery/promociones/update/${promocionSelected.id}`,
        dataToSend,
        {
          headers: {
      ...
      if (esExito) {
        ToastAlert(
          "success",
          response.data?.message || "Promoción actualizada correctamente",
        );
        ...
        ToastAlert(
          "error",
          response.data?.message || "No se pudo actualizar la promoción",
        );
      }
    } catch (error) {
      const errorMessage =
      ...
      <div className="mb-4">
        ...
        <div className="border border-2 border-dashed rounded-3 p-4 text-center bg-light">
          ...
          <p className="mb-0 text-muted small">
            Arrastra una nueva imagen o haz clic para subir y reemplazar la
            actual
          </p>
          <input
            type="file"
